...
func set_class_visuals(new_class: int) -> void:
	...
	elif new_class == PlayerMovement.PlayerClass.HOARDER:
		_build_hoarder(body_layer)
	elif new_class == PlayerMovement.PlayerClass.FRESHMAN:
...
func _build_engineer(body_layer: int) -> void:
	...
	var torso := MeshInstance3D.new()
	var t_mesh := BoxMesh.new()
	t_mesh.size = Vector3(0.42, 0.9, 0.28)
	torso.mesh = t_mesh
	torso.position.y = 0.15
	torso.material_override = hazmat_mat
	torso.layers = body_layer
	...
	var pack := MeshInstance3D.new()
	var p_mesh := BoxMesh.new()
	p_mesh.size = Vector3(0.4, 0.55, 0.2)
	pack.mesh = p_mesh
	pack.position = Vector3(0, 0.2, 0.2)
	pack.material_override = dark_mat
	pack.layers = body_layer
	add_child(pack)
	...
	al_mesh.height = 0.7
	arm_l.mesh = al_mesh
	arm_l.position = Vector3(-0.31, 0.2, 0)
	arm_l.rotation_degrees.z = -15.0
	arm_l.material_override = hazmat_mat
	arm_l.layers = body_layer
	add_child(arm_l)
	...
	ar_mesh.height = 0.7
	arm_r.mesh = ar_mesh
	arm_r.position = Vector3(0.31, 0.2, 0)
	arm_r.rotation_degrees.z = 15.0
	arm_r.material_override = hazmat_mat
	arm_r.layers = body_layer
	add_child(arm_r)
	...
	ll_mesh.height = 0.9
	leg_l.mesh = ll_mesh
	leg_l.position = Vector3(-0.13, -0.6, 0)
	leg_l.material_override = hazmat_mat
	leg_l.layers = body_layer
	add_child(leg_l)
	...
	lr_mesh.height = 0.9
	leg_r.mesh = lr_mesh
	leg_r.position = Vector3(0.13, -0.6, 0)
	leg_r.material_override = hazmat_mat
	leg_r.layers = body_layer
	add_child(leg_r)
...
func _build_hoarder(body_layer: int) -> void:
	...
	var torso := MeshInstance3D.new()
	var t_mesh := BoxMesh.new()
	t_mesh.size = Vector3(0.55, 0.9, 0.4)
	torso.mesh = t_mesh
	torso.position.y = 0.15
	torso.material_override = hazmat_mat
	torso.layers = body_layer
	...
	var pack := MeshInstance3D.new()
	var p_mesh := BoxMesh.new()
	p_mesh.size = Vector3(0.6, 0.8, 0.45)
	pack.mesh = p_mesh
	pack.position = Vector3(0, 0.25, 0.4)
	pack.material_override = dark_mat
	pack.layers = body_layer
	add_child(pack)
	...
	al_mesh.height = 0.7
	arm_l.mesh = al_mesh
	arm_l.position = Vector3(-0.38, 0.2, 0)
	arm_l.rotation_degrees.z = -20.0
	arm_l.material_override = hazmat_mat
	arm_l.layers = body_layer
	add_child(arm_l)
	...
	ar_mesh.height = 0.7
	arm_r.mesh = ar_mesh
	arm_r.position = Vector3(0.38, 0.2, 0)
	arm_r.rotation_degrees.z = 20.0
	arm_r.material_override = hazmat_mat
	arm_r.layers = body_layer
	add_child(arm_r)
	...
	ll_mesh.height = 0.9
	leg_l.mesh = ll_mesh
	leg_l.position = Vector3(-0.16, -0.6, 0)
	leg_l.material_override = hazmat_mat
	leg_l.layers = body_layer
	add_child(leg_l)
	...
	lr_mesh.height = 0.9
	leg_r.mesh = lr_mesh
	leg_r.position = Vector3(0.16, -0.6, 0)
	leg_r.material_override = hazmat_mat
	leg_r.layers = body_layer
	add_child(leg_r)
...
func _build_freshman(body_layer: int) -> void:
	...
	var torso := MeshInstance3D.new()
	var t_mesh := BoxMesh.new()
	t_mesh.size = Vector3(0.28, 0.8, 0.22)
	torso.mesh = t_mesh
	...
	# Head (Looks big on scrawny body)
	var head := MeshInstance3D.new()
	var h_mesh := BoxMesh.new()
	h_mesh.size = Vector3(0.36, 0.35, 0.3)
	head.mesh = h_mesh
	...
	v_mesh.size = Vector3(0.29, 0.12, 0.05)
	visor.mesh = v_mesh
	visor.position = Vector3(0, 0.7, -0.16)
	visor.material_override = visor_mat
	visor.layers = body_layer
	add_child(visor)
	...
	var pack := MeshInstance3D.new()
	var p_mesh := BoxMesh.new()
	p_mesh.size = Vector3(0.2, 0.4, 0.15)
	pack.mesh = p_mesh
	pack.position = Vector3(0, 0.2, 0.18)
	pack.material_override = dark_mat
	pack.layers = body_layer
	add_child(pack)
	...
	al_mesh.height = 0.65
	arm_l.mesh = al_mesh
	arm_l.position = Vector3(-0.22, 0.15, 0)
	...
	ar_mesh.height = 0.65
	arm_r.mesh = ar_mesh
	arm_r.position = Vector3(0.22, 0.15, 0)
	...
	ll_mesh.height = 0.8
	leg_l.mesh = ll_mesh
	leg_l.position = Vector3(-0.08, -0.6, 0)
	leg_l.material_override = hazmat_mat
	leg_l.layers = body_layer
	add_child(leg_l)
	...
	lr_mesh.height = 0.8
	leg_r.mesh = lr_mesh
	leg_r.position = Vector3(0.08, -0.6, 0)
	leg_r.material_override = hazmat_mat
	leg_r.layers = body_layer
	add_child(leg_r)
